...
+ Fixed some messages being out-of-order when you search a corpse.
...
+ Fixed problem where Shifter's Hong Kong UC keypad fix would cause the keypad input window to stay onscreen even after you input the correct code.
...
+ Added code to prevent NPCs from getting semi-permanently stuck inside some doors, like Manderley's office door.
...
+ Hong Kong military police hate you with fewer provocations now.
...
+ The agents in the secret lab in Hong Kong should now actually stay secretive until you blow their cover. Deus Ex 2.0's fix still had them reacting to things and noticing the player before they should have.
...
+ The Microfibral Muscle aug should be a little more useful at higher levels now: Some item weights have been altered, pushing a stack of objects now checks the weight of the whole stack instead of just the bottom object, and maximum push/lift and throwing speed calculations have been altered as well, so that the difference between aug levels is more noticeable.
...
+ Restored unused gray dissection chart image to an appropriate datacube.

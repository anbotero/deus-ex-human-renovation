...
auto state Active
{
	function TakeDamage(int Damage, Pawn EventInstigator, vector HitLocation, vector Momentum, name DamageType)
	{
		...
		//G-Flex: don't take damage from the prod either
		//G-Flex: unless we're explosive
		if ((DamageType == 'EMP') || (DamageType == 'NanoVirus') || (DamageType == 'Shocked') || ((DamageType == 'Stunned') && !bExplosive))
			return;

		if (DamageType == 'HalonGas')
		{
			ExtinguishFire();
			//G-Flex: don't be destroyed by inert gas
			return;
		}

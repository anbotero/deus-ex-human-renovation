...
//G-Flex: make persistent across maps
...
event TravelPostAccept()
{
	...
	if (AugmentationSystem != None)
	{
		...
		{
			VisionAug = AugVision(AugmentationSystem.FindAugmentation(class'AugVision'));
...
{
	//G-Flex: Damage calculation has been changed.
	//G-Flex: Base damage multiplier is slightly higher now to offset Athletics
	//G-Flex: Safe fall damage for legs lowered a tiny amount for same reason
	//G-Flex: Aug now reduces damage by a percentage (was flat amount), plus a very token flat amount
	//G-Flex: Skill reduces damage by a flat amount (smaller than old speed aug)
	...
	if (Velocity.Z < -1.4 * JumpZ)
	{
		...
		if ((Velocity.Z < -675) && (ReducedDamageType != 'All'))
			if ( Role == ROLE_Authority )
            {
				skillReduce = 0.0;
				if (SkillSystem != None)
				...
				augReduceMult = 1 - (0.125 * (augLevel+1));

				dmg = Max(augReduceMult*(-0.18*(Velocity.Z+675) - skillReduce - (2*(AugLevel + 1))), 0);
				...
				dmg = Max(augReduceMult*(-0.07*(Velocity.Z+700) - (0.75*skillReduce) - (AugLevel + 1)), 0);
...
{
	local float augValue, swimValue, speed;

	//G-Flex: Store "swimming" skill
	if (SkillSystem == None)
		swimValue = 1.0;
	else
		swimValue = SkillSystem.GetSkillLevelValue(class'SkillSwimming');
	...
	//Apply swimming skill and augmentation
	if (( Level.NetMode != NM_Standalone ) && Self.IsA('Human') )
		speed = Human(Self).mpGroundSpeed * (0.2 * swimValue - 0.2 + augValue);
	//G-Flex: Speed aug gives 25% bonus if you're crouched/crawling
	else
	{
		if (bIsCrouching || bForceDuck)
			augValue -= (augValue - 1)*0.75;

		speed = Default.GroundSpeed * (0.2 * swimValue - 0.2 + augValue);
	}
	...
	function ProcessMove ( float DeltaTime, vector newAccel, eDodgeDir DodgeMove, rotator DeltaRot)
	{
      ...
      // crouching makes you two feet tall
		...
		if ( Level.NetMode == NM_Standalone )
		{
			...
			//G-Flex: Torso health matters a little bit more for speed now, if it's quite low.
			if (HealthTorso < 25)
				newSpeed -= (defSpeed/2) * 0.10;
			else if (HealthTorso < 67)
...
exec function PutInHand(optional Inventory inv)
{
	...
	if (inv != None)
	{
		...
		//G-Flex: restrict this to single-use-only pickups
				...
				if (POVCorpse(item).carcClassString != "")
				{
					...
					if (carcClass != None)
					{
						...
						if (carc != None)
						{
							...
							carc.FamiliarName = POVCorpse(item).FamiliarName; 
...
// RipAndTear()
//
// Spacetime is huge, so it must have huge guts.
// ----------------------------------------------------------------------

exec function RipAndTear()
{
	local RipTearLauncher rayBase;
	local Vector spawnPos;
	
	if (!bCheatsEnabled)
		return;
		
	//G-Flex: spawn the thing that handles the effects
	spawnPos = Location;
	spawnPos.Z += BaseEyeHeight;
	rayBase = Spawn(class'RipTearLauncher', Self,, spawnPos, ViewRotation);
	if (rayBase != None)
		rayBase.SetBase(Self);
}

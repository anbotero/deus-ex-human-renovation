...
function Carcass SpawnCarcass()
{
		...
		if (size > 10.0)
		{
			for (i=0; i<size/4.0; i++)
			{
				...
				if (chunk != None)
				{
					...
					//G-Flex: fix so they still spread out
					//chunk.Velocity = Velocity;				
					chunk.Velocity += Velocity;
					chunk.Velocity.X += (1-2*FRand()) * (Velocity.X/8.0000);
					chunk.Velocity.Y += (1-2*FRand()) * (Velocity.Y/8.0000);
					chunk.Velocity.Z += (1-2*FRand()) * (Velocity.Z/8.0000);	
				}
			}
		}
	...
	if ( carc != None )
	{
		//G-Flex: if the dude was on fire or burned to death, he's dead now
		if ((bStunned) && !bBurnedToDeath)
			carc.bNotDead = True;
                        ...
                        bool bPlayAnim)
{
	...
	if (Health <= 0)
	{
		...
		//G-Flex: darken by the new proper amount
		if ((DamageType == 'Flamed') || bWasOnFire)
		{
			bBurnedToDeath = true;
	...
	function FindBestSeat()
	{
		...
		else
		{
			...
			if (bestSeat == None)
			{
				bestDist = 10001;
				bestSlot = -1;
				//G-flex: use VisibleActors() instead of RadiusActors()
				//G-Flex: this should prevent chair-seeking NPCs
				//G-Flex: from breaking into apartments for you
				foreach VisibleActors(Class'Seat', curSeat, 10000)
				{

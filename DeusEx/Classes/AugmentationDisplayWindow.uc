...
function DrawTargetAugmentation(GC gc)
{
	...
	//G-Flex: makes something more convenient later
	local bool bShowHealth;
   ...
   targetplayerlocationstring = "";
	...
	if ( target != None )
	{
		...
		if ((weapon != None) && !weapon.bHandToHand && !bUseOldTarget)
		{
			...
			//G-Flex: also don't draw if the target is a corpse
			if ((weapon.MaxRange >= VSize(target.Location - Player.Location)) && !target.IsA('DeusExCarcass'))
			{
				w = width;
				h = height;
				x = int(w * 0.5)-1;
				...
				//G-Flex: it effectively "clamps" a different accuracy depending on those values
				//G-Flex: but this doesn't matter much because you hardly get under 0.1 acc anyway
				//G-Flex: unless your accuracy is perfect, and those give a reticle size at/above the
				//G-Flex: minimum size for pretty much any res above 800x600
				//G-Flex: (I tried correcting it such that the minimum reticle size is that of
				//G-Flex: 0.1625acc like in 640x480, but it looks worse, and doesn't
				//G-Flex: really make a difference unless you're comparing to such low res.
				//G-Flex: that calculation is below for reference)
				//mult = FMax(mult, ((corner+4.0) * (width / 640) * (accuracyMultiplier / 80));
				...
				//log(weapon.currentAccuracy);
				//log(mult);
				for (i=1; i>=0; i--)
				{
					//G-Flex: drawing boxes with non-round floats leads to odd behavior
					mult = Int(mult);
					//G-Flex: fixed some off-by-one errors below by adding 1 to two coords
					gc.DrawBox(x+i, y-mult+i, 1, corner, 0, 0, 1, Texture'Solid');
					gc.DrawBox(x+i, y+1+mult-corner+i, 1, corner, 0, 0, 1, Texture'Solid');
					gc.DrawBox(x-(corner-1)/2+i, y-mult+i, corner, 1, 0, 0, 1, Texture'Solid');
					gc.DrawBox(x-(corner-1)/2+i, y+mult+i, corner, 1, 0, 0, 1, Texture'Solid');

					gc.DrawBox(x-mult+i, y+i, corner, 1, 0, 0, 1, Texture'Solid');
					gc.DrawBox(x+1+mult-corner+i, y+i, corner, 1, 0, 0, 1, Texture'Solid');
	...
	if (target != None)
	{
		...
		if (ConvertVectorToCoordinates(target.Location, boxCX, boxCY))
		{
			...
			if ((bTargetActive) && (Player.Level.Netmode == NM_Standalone))
			{
				...
				//G-Flex: Allow targeting of bodies, use familiar name
